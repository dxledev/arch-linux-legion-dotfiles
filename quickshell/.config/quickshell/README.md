...
The bottom launcher's Commands → Theme entry opens a searchable theme submenu beside Wallpaper. Names and palette wheels reuse `~/bin/menu-theme`'s parser; the current theme has a checkmark. Selecting a theme runs `~/bin/theme`, while selecting the current theme only closes the launcher. `caelestia/integration/theme-list --dry-run` lists themes without generating icons. Override the parser with `MENU_THEME_SCRIPT`; its `MENU_THEME_ALT_ICON_*` settings also apply.
