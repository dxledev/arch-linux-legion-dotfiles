...
Item {
  ...
  function startPhase(nextPhase) {
    ...
    phase = nextPhase
    stderrText = ""
    speedTestProc.command = ["/usr/bin/bash", Quickshell.shellPath("integration/network-speedtest"), nextPhase]
  ...
  Process {
    id: statusProc
    command: ["/usr/bin/nmcli", "-t", "--escape", "no", "-f", "TYPE,NAME", "connection", "show", "--active"]
      ...
      onStreamFinished: {
        ...
        for (var line of lines) {
          if (line.startsWith("802-11-wireless:")) {
            root.connectionName = line.slice(line.indexOf(":") + 1)
            return
          }
          if (line.startsWith("802-3-ethernet:")) root.connectionName = "Ethernet"
        }
      }
    }
  }
