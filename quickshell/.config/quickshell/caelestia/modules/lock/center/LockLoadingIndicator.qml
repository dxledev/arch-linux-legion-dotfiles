...
MaterialShape {
    ...
    FrameAnimation {
        ...
        onTriggered: {
            ...
            if (elapsed >= root.springDuration) {
                root.springSettled = true;
            } else {
                const [position, velocity] = root.spring(elapsed);
                root.morphProgress = Math.min(1, position);
                root.thisLRotation = position * root.morphAnimRotation;
                root.scale = 1 + velocity * root.morphScale / root.springMaxVelocity;
            }
        }
    }
